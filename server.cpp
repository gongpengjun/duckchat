...
int sayReq(struct request_say *rs)
{
    ...
    for(int i=0; i<tmpU.size(); i++) {
        ...
        struct text_say *msg = (struct text_say*) malloc(sizeof(struct text_say) + (sizeof(struct channel_info)));
        ...
        int res= sendto(sockfd, msg, (sizeof(struct text_say) + (sizeof(struct channel_info))), 0, (struct sockaddr*)&address, size);
        if (res == -1) {
